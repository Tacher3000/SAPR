...
void MainWindow::updateScene()
{
    ...
    for (int row = 0; row < model->rowCount(); ++row) {
        ...
        if (direction != 0) {
            ...
            for (int i = 0; i < arrowCount; ++i) {
                ...
                QLineF line(arrowX, arrowY, arrowX + (direction > 0 ? arrowSpacing : -arrowSpacing), arrowY);
    ...
    if(settings->value("checkBoxNode", false).toBool())
    {
        drawNode(maxHeight);
    }
...
void MainWindow::drawNode(qreal maxHeight)
{
    ...
    for (int row = 0; row < model->rowCount(); ++row) {
        ...
        m_scene->addRect(square, QPen(Qt::black), QBrush(Qt::NoBrush));
...
void MainWindow::drawKernel(qreal maxHeight)
{
    ...
    for (int row = 0; row < model->rowCount() - 1; ++row) {
        ...
        m_scene->addEllipse(circle, QPen(Qt::black), QBrush(Qt::NoBrush));
